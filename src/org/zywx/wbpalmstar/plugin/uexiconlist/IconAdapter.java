...
public class IconAdapter implements SAdapter, ConstantUtils {
	...
	public IconAdapter(Context context, LinkedList<IconBean> list, String defIconUrl, String widgetInfo,
			EUExIconList mEUExIconList, ScrollLayout mContainer) {
		mContext = context;
	...
	public View getView(int position) {
		...
		if (position < mIconList.size()) {
			...
			convertView = mInflater.inflate(EUExUtil.getResLayoutID("plugin_iconlist_item"), null);
			final IconViewHolder hoder = new IconViewHolder();
			hoder.ivIcon = (ImageView) convertView.findViewById(EUExUtil.getResIdID("plugin_iconlist_ivIcon"));
			int iconBTPadding = (int) (UIConfig.getScaleHight() * ICON_PADDING_HIGHT_SCALE);
			LinearLayout.LayoutParams lParams = (LinearLayout.LayoutParams) hoder.ivIcon.getLayoutParams();
			lParams.width = UIConfig.getIconScaleWidth();
			lParams.height = UIConfig.getIconScaleHight();
			...
			hoder.tvIconName = (TextView) convertView.findViewById(EUExUtil.getResIdID("plugin_iconlist_tvTitle"));
			LinearLayout.LayoutParams llpParams = (LinearLayout.LayoutParams) hoder.tvIconName.getLayoutParams();
			llpParams.height = UIConfig.getTitleScaleHight();
			hoder.tvIconName.setLayoutParams(llpParams);

			int textSize = DensityUtil.px2sp(mContext, UIConfig.getTitleScaleHight() * TITLE_TEXT_HIGHT_SCALE);
			hoder.tvIconName.setTextSize(textSize);
			hoder.tvIconName.setTextColor(UIConfig.getTileTextColor());
			hoder.ivDelIcon = (ImageView) convertView.findViewById(EUExUtil.getResIdID("plugin_iconlist_del_app_btu"));
			Bitmap bitmap = null;
			try {
				if (iconInfo.getIcon().startsWith("http://")) {
					...
					if (null == bitmap) {
						imgLoadMgr.asyncLoad(new ImageLoadTask(iconInfo.getIcon()) {
							@Override
							protected Bitmap doInBackground() {
								Bitmap mBitmap = IconListUtils.downloadNetworkBitmap(iconInfo.getIcon());
								return mBitmap;
							}
					...
					bitmap = IconListUtils.loadBitmapByUrl(mContext, iconInfo.getIcon(), defIconUrl, widgetJson);
					hoder.ivIcon.setImageBitmap(bitmap);
				}
			} catch (Exception e) {
				e.printStackTrace();
				...
				public void onClick(View v) {
					if (!mContainer.isEditting()) {
						if (!ScrollLayout.isMove) {
							mEUExIconList.mEuExIconListHandler.send2Callback(WHAT_ON_ICON_CLICK, iconInfo.getJsonStr());
						}
					}
				}
			});
